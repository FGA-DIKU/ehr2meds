...
set -e
# bash -i run_tabs.sh ../MEDS_data/ ../MEDS_data_reshard/ ../MEDS_data_tabs/ 'cancer/all_cancers/' ../MEDS_data/task_labels ../models/ 200
...
meds-tab-tabularize-static \
    "input_dir=${MIMICIV_MEDS_RESHARD_DIR}/data" "output_dir=$OUTPUT_TABULARIZATION_DIR" \
    do_overwrite=False "$@"\
    tabularization.aggs=["static/present"]
...
do
    ...
    "tabularization.aggs=[static/present,code/count]" \
    "tabularization.window_sizes=[1d,7d,30d,365d,full]"
