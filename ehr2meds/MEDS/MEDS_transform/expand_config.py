...
def get_files_in_directory(
    dir_path: Path, extensions: tuple = (".csv", ".parquet")
) -> list[str]:
...
def expand_config_for_directories(
    config: Dict[str, Any], input_dir: Path, extensions: tuple = (".csv", ".parquet")
) -> Dict[str, Any]:
    ...
    for concept_name, concept_config in config.items():
        ...
        if concept_path.exists() and concept_path.is_dir():
            ...
            if files:
                ...
                print(
                    f"Warning: '{concept_name}' is a directory but contains no {extensions} files"
                )
...
def main():
    ...
    parser.add_argument(
        "config_file", type=str, help="Path to the input YAML config file"
    )
    parser.add_argument(
        "-o",
        "--output",
