...
def dataset(
    ...
    version=None,
    overwrite_ok=False,
    post_validate_compare=True,
    post_validate_func=None,
):
    ...
    log().info(
        f"Loaded {name}:{ds.version}, got {len(df)} rows with {len(df.columns)} columns!"
    )
    ...
    if not overwrite_ok:
        ...
        try:
            ds = Dataset.Tabular.from_parquet_files(
                (dast, full_remote_path + "/data.parquet")
            )
            found = True
            ...
            if not e.error_code == "ScriptExecution.StreamAccess.NotFound":
                ...
                raise Exception(
                    f"Looking for dataset @ {(_BACKUP_DATASTORE,full_remote_path)} caused an unexpected exception: {e}."
                )

        if found:
            raise Exception(
                f"Cannot make backup of {name}, backup {(_BACKUP_DATASTORE,full_remote_path)} already exists. Set ovewrite_ok=True to ignore!"
            )
    ...
    try:
        ...
        log().error(
            f"Failed to backup dataset: {name}:{ds.version} to {(dast,full_remote_path)}!"
        )
        raise e
    finally:
        # Clean-up
