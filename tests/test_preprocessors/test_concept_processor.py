...
class TestConceptProcessor(unittest.TestCase):
    ...
    def test_process_codes_prefix_and_fill(self):
        ...
        df = pd.DataFrame({CODE: [None, "ABC"], "fill_code": ["X", "Y"]})
        concept_config = {
            "fillna": {CODE: {"column": "fill_code", "regex": None}},
            "code_prefix": "PRE_",
        }
    ...
    def test_merge_admissions(self):
        ...
        self.assertEqual(
            merged_1.iloc[0][ADMISSION], pd.to_datetime("2022-01-01 00:00")
        )
        self.assertEqual(
            merged_1.iloc[0][DISCHARGE], pd.to_datetime("2022-01-01 14:00")
        )
        # For subject 2, the row remains unchanged.
        merged_2 = result[result[SUBJECT_ID] == 2]
        self.assertEqual(len(merged_2), 1)
        self.assertEqual(
            merged_2.iloc[0][ADMISSION], pd.to_datetime("2022-01-02 00:00")
        )
    ...
    def test_combine_datetime_columns(self):
        ...
        config_drop = {
            "combine_datetime": {
                "timestamp": {"date_col": "date_col", "time_col": "time_col"}
            }
        }
        ...
        config_missing = {
            "combine_datetime": {
                "timestamp": {"date_col": "missing_col", "time_col": "time_col"}
            }
        }

        result_missing = ConceptProcessor._combine_datetime_columns(
            df.copy(), config_missing
        )
        ...
        config_multi = {
            "combine_datetime": {
                "admission": {"date_col": "date_start", "time_col": "time_start"},
                "discharge": {"date_col": "date_end", "time_col": "time_end"},
            }
        }

        result_multi = ConceptProcessor._combine_datetime_columns(
            df_multi.copy(), config_multi
        )
    ...
    def test_convert_numeric_columns(self):
        ...
        config = {"numeric_columns": ["col1", "col2"]}
        ...
        result_empty = ConceptProcessor._convert_numeric_columns(
            df.copy(), config_empty
        )
        ...
        result_missing = ConceptProcessor._convert_numeric_columns(
            df.copy(), config_missing
        )
    ...
    def test_unroll_columns(self):
        ...
        config = {
            "columns_map": {"original_timestamp": "timestamp"},
            "unroll_columns": [
                {"column": "specialty", "prefix": "SPEC_"},
                {"column": "unit_type", "prefix": "UNIT_"},
            ],
        }
        ...
        config_no_prefix = {
            "columns_map": {"original_timestamp": "timestamp"},
            "unroll_columns": [{"column": "specialty"}],
        }
    ...
    def test_map_and_clean_data(self):
        ...
        result_no_subject = ConceptProcessor._map_and_clean_data(
            df_no_subject.copy(), subject_mapping
        )
        ...
        result_dupes = ConceptProcessor._map_and_clean_data(
            df_dupes.copy(), subject_mapping
        )
        ...
        result_unknown = ConceptProcessor._map_and_clean_data(
            df_unknown.copy(), subject_mapping
        )
