...
from MEDS_preprocess.preprocessors.azure import AzurePreprocessor
import torch
from MEDS_preprocess.preprocessors.azure_load import datastore
import numpy as np
...
class Normaliser:
    ...
    def __init__(self, cfg, logger) -> None:
        ...
        if "dist_path" not in cfg.data:
            ...
            self.vocab = torch.load(
                join(mount_point, "vocabulary.pt"), weights_only=True
            )
    ...
    def __call__(self):
        ...
        if not Path(join(cfg.paths.output_dir, save_name)).exists():
            ...
            for chunk in tqdm(
                self.azure_processor.load_chunks(cfg.data), desc="Chunks"
            ):
                ...
                if counter == 0:
                    self.azure_processor.save(
                        chunk_processed, cfg.data, f"concept.{save_name}", mode="w"
                    )
                else:
                    self.azure_processor.save(
                        chunk_processed, cfg.data, f"concept.{save_name}", mode="a"
                    )
